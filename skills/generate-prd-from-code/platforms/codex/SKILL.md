...
description: Reverse-engineer an existing repository into a business-first PRD for initialization and AI onboarding using native repository exploration and Codex browser tools when available. If a paired frontend repo, paired backend repo, or running frontend URL is likely needed but missing, ask the user whether to supplement those inputs or explicitly proceed with a limited current-repo-only PRD.
---
...
This Codex adapter is agent-native:

- inspect repository files directly
- use browser or Chrome MCP only when the user provides a live frontend URL
...
1. open the URL with Chrome MCP or other available browser tools in Codex
...
If some visible pages cannot be opened or fully captured, keep them as menu-level placeholders and mark them as blocked.
