...
2. Prefer opening the provided URL with Chrome MCP. If Chrome MCP is unavailable in the current Claude Code session, fall back to another available webpage-reading or browser MCP.
...
8. Save the frontend evidence to a JSON file that matches the evidence rules.
...
If Chrome MCP is unavailable in the current Claude Code session, you may fall back to another available webpage-reading or browser MCP, but say explicitly that Chrome MCP was unavailable and that fallback evidence may be shallower than a full interactive browser capture.
